...
def load_torchMnist():
    ...
    tr_x = cp.asarray(tr_x, dtype=cp.float32).reshape(60000, -1)
    print(f"[INFO]\t(cupy)tr_x:  {tr_x.shape}")

    tr_y = cp.asarray(tr_y, dtype=cp.int8)
    print(f"[INFO]\t(cupy)tr_y: {tr_y.shape}")

    ts_x = cp.asarray(ts_x, dtype=cp.float32).reshape(10000, -1)
...
def load_Cifar100():
    ...
    tr_x = cp.asarray(tr_x.transpose(0,3,1,2), dtype=cp.float32)
    print(f"[INFO]\t(cupy)tr_x:  {tr_x.shape}")

    tr_y = cp.asarray(tr_y, dtype=cp.int8)
    print(f"[INFO]\t(cupy)tr_y: {tr_y.shape}")

    ts_x = cp.asarray(ts_x.transpose(0,3,1,2), dtype=cp.float32)
    print(f"[INFO]\t(cupy)ts_x: {ts_x.shape}")
...
def load_SVHN():
    ...
    tr_x = cp.asarray(tr_x, dtype=cp.float32)
    print(f"[INFO]\t(cupy)tr_x:  {tr_x.shape}")

    tr_y = cp.asarray(tr_y, dtype=cp.int8)
    print(f"[INFO]\t(cupy)tr_y: {tr_y.shape}")

    ts_x = cp.asarray(ts_x, dtype=cp.float32)
    print(f"[INFO]\t(cupy)ts_x: {ts_x.shape}")
