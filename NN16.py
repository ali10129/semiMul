...
extern "C"
__inline__ __device__ __half semiMul(__half a, __half b) 
{  
    unsigned short d1 = __half_as_ushort(a);
    unsigned short d2 = __half_as_ushort(b);

    unsigned short d3 = 0;

    unsigned char ed1 = ((d1) >> 10) & 0x1f;
    unsigned char ed2 = ((d2) >> 10) & 0x1f;

    bool underflow = (ed1 == 0) | (ed2 == 0) | (ed1 + ed2 <= 15);
    bool overflow = ((ed1 + ed2) >= (31 + 15));

    if (underflow)
        d3 = 0;
    else if (overflow)
        d3 = ((d1 ^ d2) | (31 << 10)) & ((32 + 31) << 10);
    else
        d3 = (d1 + d2) - (15 << 10);

    return __ushort_as_half(d3);
}
//////////////////////////////////////////////////////////////////////////////
extern "C" __global__
void semiHadamardScalar(__half Scaler, __half * A, __half * Out, int H, int W)
{
...
////////////////////////////////////////////////////////////////////////////
extern "C" __global__
void semiHadamard(__half * A, __half * B, __half * C, int H, int W)
{
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
...
extern "C" __global__
void semiMAC(__half * A, __half * B, __half * C, int m, int k, int n)
{
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    {
        //Create 2 tiles for matrix A and B at the shared memory
        __shared__ __half ATile[TILE_DIM][TILE_DIM];
        __shared__ __half BTile[TILE_DIM][TILE_DIM];
        ...
        //calculate a partial value of thread element in C
        static_assert(__CUDA_ARCH__ >= 530,"\n\n===>>>\t[Device error during Compile] __CUDA_ARCH__ < 530, so  __hadd is not properly defined for you gpu! please use another gpu architecture\n\n");
#pragma unroll
        for (int i = 0; i < TILE_DIM; i++){
            elementC = __hadd(semiMul( ATile[thrY][i] , BTile[i][thrX] ), elementC);
        }
        __syncthreads();
...
def iHads(scalar, A, xOut):
    ...
    DimGrid = (W, H, 1)
    DimBlock = (1, 1, 1)
    semiHadamardScalar(DimGrid, DimBlock, (cp.float16(scalar), A, xOut, H,W))  
    return xOut
